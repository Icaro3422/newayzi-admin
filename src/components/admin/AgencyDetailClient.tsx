...
export function AgencyDetailClient() {
  ...
  if (!agency) {
    return (
      <div className="rounded-lg border border-semantic-surface-border bg-white p-6">
        <p className="text-semantic-text-muted">Agencia no encontrada.</p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href="/admin/agents"
          className="flex items-center gap-1 text-sm text-semantic-text-muted hover:text-newayzi-han-purple"
        >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="border border-semantic-surface-border">
          <CardHeader className="pb-0">
            <span className="text-sm font-medium text-semantic-text-muted">Nivel</span>
          </CardHeader>
        ...
        <Card className="border border-semantic-surface-border">
          <CardHeader className="pb-0">
            <span className="text-sm font-medium text-semantic-text-muted">Ventas totales</span>
          </CardHeader>
          <CardBody className="pt-2">
        ...
        <Card className="border border-semantic-surface-border">
          <CardHeader className="pb-0">
            <span className="text-sm font-medium text-semantic-text-muted">Comisión generada</span>
          </CardHeader>
          <CardBody className="pt-2">
        ...
        <Card className="border border-semantic-surface-border">
          <CardHeader className="pb-0">
            <span className="text-sm font-medium text-semantic-text-muted">Reservas</span>
          </CardHeader>
          <CardBody className="pt-2">
      ...
      <Card className="border border-semantic-surface-border">
        ...
        <CardBody className="pt-0">
          <dl className="grid gap-2 sm:grid-cols-2">
            <div>
              <dt className="text-sm text-semantic-text-muted">Ventas totales (reservas confirmadas)</dt>
              <dd className="font-medium">{formatCurrency(s.total_sales)}</dd>
            </div>
            <div>
              <dt className="text-sm text-semantic-text-muted">Comisión acumulada</dt>
              <dd className="font-medium">{formatCurrency(s.total_commission)}</dd>
            </div>
            <div>
              <dt className="text-sm text-semantic-text-muted">Número de reservas</dt>
              <dd className="font-medium">{s.bookings_count}</dd>
            </div>
            <div>
              <dt className="text-sm text-semantic-text-muted">Última actualización</dt>
