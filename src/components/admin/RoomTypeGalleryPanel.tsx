...
import { adminApi, type RoomTypePicture } from "@/lib/admin-api";
...
interface Props {
  ...
  readOnly?: boolean;
  onPicturesChange?: (pics: RoomTypePicture[]) => void;
}

export function RoomTypeGalleryPanel({
  propertyId,
  roomTypeId,
  pictures,
  readOnly = false,
  ...
  const [pics, setPics] = useState<RoomTypePicture[]>(pictures);
  ...
  const notify = (next: RoomTypePicture[]) => {
    setPics(next);
    onPicturesChange?.(next);
  };

  const refreshPics = useCallback(async () => {
    const updated = await adminApi.getRoomTypePictures(propertyId, roomTypeId);
    notify(updated);
  ...
  const handleSetPrimary = async (pic: RoomTypePicture) => {
    if (pic.is_primary || readOnly) return;
    setPrimaryId(pic.id);
    try {
      await adminApi.setRoomTypePicturePrimary(propertyId, roomTypeId, pic.id);
      await refreshPics();
  ...
  const handleDelete = async (pic: RoomTypePicture) => {
    if (readOnly) return;
    setDeletingId(pic.id);
    try {
      await adminApi.deleteRoomTypePicture(propertyId, roomTypeId, pic.id);
      await refreshPics();
  ...
  return (
    <div className="space-y-4">
      {!readOnly && (
        <div
          className={`relative flex flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed transition-colors duration-200 p-6 sm:p-8 cursor-pointer
      ...
      {pics.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-10 text-white/30">
          ...
          <p className="text-sm text-center px-4">
            {readOnly ? "Este tipo no tiene imágenes cargadas." : "Aún no hay imágenes. Sube la primera."}
          </p>
        </div>
      ) : (
